...
| 4 | **Cross-response analysis, no new request** | Method/endpoint-equivalent columns are blank AND a `tc_ref`/comparison column names one or more earlier `tc_id`s | Response-cache mechanism (§0) + `pm.execution.skipRequest()`; §4 |
