...
| `tc_ref` non-empty | This row does not fire a new HTTP request. It is an analysis-only row that inspects or diffs the response(s) already captured for the referenced `tc_id`(s) (TC-29, TC-30, TC-31, TC-32, TC-35). |
...
account's session. Rows referencing `tc_ref` (TC-29..TC-32, TC-35) must run after every
`tc_id` they reference has already produced a captured response.
...
Rows TC-29, TC-30, TC-31, TC-32, TC-35 (cross-response inspection / comparison) are not
sent as new requests by this template — they are a post-run analysis step over the
responses already captured for the `tc_id`(s) named in their `tc_ref` column, executed
once the full data file has run.
