...
| | Workflow của mình | Workflow đề xuất cho bạn |
| - | ------------------- | -------------------------- |
| Auth-heavy | POST /api/login (khách) | POST /api/login (admin) |
| Read-heavy | GET /api/products?search= | GET /api/admin/orders |
| Transactional | POST /api/cart + POST /api/checkout | PUT /api/admin/orders/:id/status |
| Narrative | Khách mua hàng | Vận hành admin |