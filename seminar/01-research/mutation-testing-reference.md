...
- **Mutation testing is the only adequacy metric that scores both execution *and* assertion quality.** LLM-generated tests can reach 100% line and branch coverage with a mutation score of only 4% (Wang et al., MUTGEN, IEEE TSE accepted / arXiv:2506.02954 — measured on subject `id_81` of HumanEval-Java, tests generated by Llama-3.3). Treat mutation score — not coverage — as the test-suite quality KPI. Source URLs for every citation below: see [§ Source Index](#source-index).
...
- **AI now plugs into mutation testing at three points**: (1) generating tests targeted at surviving mutants (MuTAP achieves **93.57%** mutation score on HumanEval vs **65.94%** for Pynguin; Dakhel et al., IST 2024); (2) generating *better* mutants than fixed-operator engines (LLMorpheus, Tip, Bell & Schäfer, arXiv:2404.09952); (3) screening equivalent mutants (Tian et al., ISSTA 2024 — best F1-score 86.58% with fine-tuned UniXCoder, a 35.69% F1 improvement over prior techniques). All three carry hallucination risk and require executable verification — never accept an LLM-generated assertion without running it against the original code AND the live mutant.
...
- ⚠️ **UNVERIFIED — do not present as research evidence.** "Theodo (Diffblue case study, banking microservice): 96% line / 93% branch coverage but only 34% mutation score." A primary URL for this case could not be located (checked 30 Jul 2026). Descriptions matching it (PIT/Java, 114 mutants, 75 surviving) circulate only on consultancy blogs, not in peer-reviewed literature. **Peer-reviewed replacement available:** *Mutation Testing as a Quality Assurance Technique in a Fintech Company: An Experience Report in The Industry*, SBQS 2024 (authors not yet verified) — use that instead, or drop the claim entirely in favour of the team's own measured e-Shop numbers.
- Wang et al., MutGen (IEEE TSE accepted / arXiv:2506.02954): "Some test suites achieve 100% coverage but only 4% mutation score." Verified in the full text: this is subject `id_81` of HumanEval-Java, tests generated by Llama-3.3 70B — **a single subject in the §II-A motivating example, not a benchmark average**. The vanilla-prompting average across subjects is 77.9% / 69.9%, so never present 4% as the baseline. Stronger companion figure from Table I: across all four approaches on the same subjects, line coverage spans under 3 percentage points while mutation score spans 22–31 points — and on Leetcode-Java the highest-coverage approach (EvoSuite, 99.0%) has the lowest mutation score (58.9%).
...
- **MuTAP** (Dakhel et al., *Information & Software Technology* 2024 / arXiv:2308.16557; code: `ExpertiseModel/MuTAP`). Pipeline: (1) zero/few-shot prompt for initial unit tests; (2) syntactic repair; (3) oracle correction by comparing actual vs expected return values; (4) run mutation testing; (5) re-prompt with surviving mutants until none remain or budget exhausted. **93.57% mutation score on HumanEval** (vs **65.94%** for Pynguin; best config = few-shot, llama-2-chat); on the Refactory benchmark of real buggy submissions, **94.91%** vs **67.54%** for Pynguin. Quote the benchmark name with the number — the paper reports two different pairs and they are easy to confuse. The "oracle minimizer" drops assertions that don't increase MS — typically yielding 2–3 high-impact assertions per function.
- **MutGen / MUTGEN** (Wang, Xu, Briand, Liu — **accepted in IEEE Transactions on Software Engineering**; arXiv:2506.02954). Embeds **mutation-report feedback** (line, status, operator) directly in the prompt; adds a fixing step for assertion failures. On 204 subjects from two benchmarks (104 HumanEval-Java + 100 Leetcode-Java), MutGen outperforms EvoSuite, EvoSuite_mut and vanilla prompting on mutation score — 89.5% / 89.1% versus EvoSuite 69.5% / 58.9% and Gen_vanilla 77.9% / 69.9%, A12 effect size 0.650–0.899. Coverage, meanwhile, barely moves: all four approaches land within 3 percentage points of each other (95.4–98.3% and 96.3–99.0% line coverage). That contrast is the strongest form of the argument — coverage cannot discriminate between these suites, mutation score can. On Leetcode-Java it is sharper still: the approach with the **highest** coverage (EvoSuite, 99.0%) has the **lowest** mutation score (58.9%), so coverage ranks them in reverse. Motivating example from §II-A: "Some test suites achieve 100% coverage but only 4% mutation score" (subject `id_81`, HumanEval-Java, Llama-3.3 70B) — a single subject, quote it as such.
...
- **Meta TestGen-LLM / "Automated Unit Test Improvement Using Large Language Models at Meta"** (Alshahwan et al., **FSE 2024** / arXiv:2402.09171). Production deployment: LLM-generated test additions ship only after clearing an automated filter chain — the test class **builds**, **passes reliably** across repeated runs, and **increases coverage** over the original class.

  ⚠️ **Correction — read this before citing.** Meta's "assured improvement" signal is **coverage**, not mutation score or any fault-detection metric. Reported results: 75% of generated test cases built correctly, 57% passed reliably, 25% increased coverage; 11.5% of all classes it was applied to were improved, and 73% of its recommendations were accepted for production by Meta engineers. The transferable lesson is **"a mandatory, automated, measurable gate before any LLM-written test is merged"** — *not* "industry gates LLM tests on mutation score." Substituting mutation score for coverage inside that gate is **our proposal [I]**, and must be labelled as such.
...
The goal is to **raise mutation score without inflating LOC**. MuTAP's oracle minimizer is the canonical pattern: after each LLM-generated test, recompute MS — keep only assertions that newly kill ≥1 mutant. Empirically this caps growth at 2–3 net new assertions per function. Meta's TestGen-LLM (Alshahwan et al., FSE 2024) enforces a structurally similar discipline: every accepted test must (a) build, (b) pass reliably, (c) improve a measurable quality signal — otherwise discarded. Note the difference in *which* signal: Meta's is **coverage**; the mutation-score variant used in this project is our own tightening of their gate, not their published practice.

### 3.5 LLM-suggested mutation operators

- **LLMorpheus** (Tip, Bell, Schäfer — arXiv:2404.09952; ⚠️ the "IEEE TSE 2025" venue previously recorded here could **not** be confirmed against arXiv metadata or DBLP on 30 Jul 2026 — cite the arXiv ID until the journal record is verified; code: `githubnext/llmorpheus`). Instead of using a fixed operator set, LLMorpheus inserts `<PLACEHOLDER>` tokens at high-value AST locations (if-conditions, loop headers, call arguments) and asks an LLM to propose realistic replacements. Implemented atop a modified StrykerJS so the same reporting/HTML stack is reused. Across 13 npm packages: **produces mutants that resemble existing bugs that StrykerJS's fixed operator set cannot generate**. Best performer: `codellama-34b-instruct` at T=0. Practical caveats: cost (per-mutation API calls), nondeterminism, and the need to verify mutant validity (a non-trivial fraction are equivalent refactors).
...
| **Pynguin** | OSS, Python only | Coverage-driven; MuTAP outperforms it on mutation score (93.57% vs 65.94% on HumanEval). |
...
- **Equivalent-mutant rate.** The often-cited "4–39%" range comes from Madeyski et al. — usually cited as 2013 (online-first), formally published in **IEEE TSE 2014** as *Overcoming the Equivalent Mutant Problem: A Systematic Literature Review and a Comparative Experiment of Second Order Mutation*. Verified as quoted by Tian et al. ISSTA 2024 ("the rate of equivalent mutants in real-world development scenarios ranges from 4% to 39%"). Measured on Java/C studies; the JS/TS equivalent-mutant rate with StrykerJS's default mutators is less thoroughly measured but anecdotally lower for arithmetic/relational mutators and higher for `StringLiteral`/`ObjectLiteral`.
...
Every external claim in this document resolves to a row below. Verification status recorded **30 Jul 2026**: `✅ verified` = the exact number/wording was checked against the primary source (arXiv full text, publisher metadata, or official docs) on that date · `⚠️ unverified` = no primary source located, do not present as research evidence.

### Peer-reviewed research

| Work | Venue of record | Open URL | Verified claim | Status |
| --- | --- | --- | --- | --- |
| Wang, Xu, Briand, Liu — *Mutation-Guided Unit Test Generation with a Large Language Model* (MutGen) | IEEE TSE (accepted) | <https://arxiv.org/abs/2506.02954> | "some test suites achieve 100% coverage but only 4% mutation score" — subject `id_81`, HumanEval-Java, Llama-3.3 70B (§II-A **motivating example**, not a benchmark average). Table I, mutation score, HumanEval-Java / Leetcode-Java: EvoSuite **69.5% / 58.9%**, EvoSuite_mut **67.4% / 58.1%**, Gen_vanilla **77.9% / 69.9%**, MutGen **89.5% / 89.1%**. The published "+28.8% / +51.3% over EvoSuite" are **relative**, not percentage points (absolute: +20.0 pp / +30.2 pp). A12 effect size 0.650–0.899 | ✅ verified against Table I, 01/08/2026 |
| ↳ *same paper — the coverage-vs-mutation contrast worth citing* | | | Across all four approaches on the same subjects: line coverage spans 95.4–98.3% (HumanEval-Java) and 96.3–99.0% (Leetcode-Java) — under 3 points; mutation score spans 67.4–89.5% and 58.1–89.1% — 22 to 31 points. On Leetcode-Java the **highest-coverage** approach (EvoSuite, 99.0%) has the **lowest** mutation score (58.9%): coverage ranks the approaches in reverse | ✅ verified against Table I, 01/08/2026 |
| Banik, Chowdhury, Shamim — *All Smoke, No Alarm: Oracle Signals in Agent-Authored Test Code* | ⚠️ venue unconfirmed — cite the arXiv ID (v1, 16 Jun 2026) | <https://arxiv.org/abs/2606.18168> | 86,156 test-file patches from 33,596 agent-authored PRs across 2,807 repos, 5 agents (Codex, Copilot, Devin, Cursor, Claude Code), Dec 2024–Jul 2025. **80.2% of patches carry weak or no explicit oracle signal** (W1–W5); S1 value assertions 11.3%, S3 multi-signal 5.7%. On newly created test files, strong-oracle rate ranges **18% (Codex) → 67% (Claude Code)**, χ²=810.2, p<0.001. Manual validation: 384 stratified patches, Cohen's κ=0.77; classifier–human agreement 86.7%. Quote: *"coding agents generate test structure far more reliably than they generate oracle logic"* | ✅ verified full text 01/08/2026 |
| Yoshimoto, Fujita, Horikawa, Feitosa, Kashiwa, Iida — *Testing with AI Agents: An Empirical Study of Test Generation Frequency, Quality, and Coverage* | ⚠️ venue unconfirmed — cite the arXiv ID | <https://arxiv.org/abs/2603.13724> | AIDev dataset, 2,232 test-related commits. AI authored **16.4%** of all test-adding commits; AI-generated tests are longer, have **higher assertion density than humans** (median 2 vs 1, p<0.001), lower cyclomatic complexity, and coverage gains **comparable** to human-authored tests. **Counter-evidence to the 80.2% figure — see reconciliation note below.** Their own future work: *"mutation testing should determine if the high assertion density of AI tests translates into superior fault-detection capabilities"* | ✅ verified full text 01/08/2026 |
| Dakhel, Nikanjam, Majdinasab, Khomh, Desmarais — *Effective Test Generation Using Pre-trained LLMs and Mutation Testing* (MuTAP) | Information & Software Technology, 2024 | <https://arxiv.org/abs/2308.16557> | HumanEval: MS **93.57%** (MuTAP) vs **65.94%** (Pynguin); Refactory: **94.91%** vs **67.54%**; detects up to 28% more faulty human-written snippets | ✅ verified |
| Alshahwan et al. — *Automated Unit Test Improvement using LLMs at Meta* (TestGen-LLM) | FSE 2024 (industry track) | <https://arxiv.org/abs/2402.09171> | Filters = builds + passes reliably + **increases coverage**; 75% built, 57% passed reliably, 25% increased coverage, 11.5% of classes improved, 73% of recommendations accepted | ✅ verified — see correction in §3.5 |
| Tian, Shu, Wang, Cao, Kamei, Chen — *LLMs for Equivalent Mutant Detection: How Far Are We?* | ISSTA 2024 | <https://arxiv.org/abs/2408.01760> | Fine-tuned UniXCoder: P 94.33% / R 81.81% / **F1 86.58%**; average **+35.69% F1** over 10 prior EMD techniques; 3,302 Java mutant pairs | ✅ verified |
| Tip, Bell, Schäfer — *LLMorpheus: Mutation Testing using Large Language Models* | ⚠️ venue unconfirmed — cite the arXiv ID | <https://arxiv.org/abs/2404.09952> | Placeholder-based LLM mutation built atop StrykerJS; 13 subject packages; produces bug-resembling mutants StrykerJS cannot generate | ✅ claim verified / ⚠️ venue unverified |
| Petrović, Ivanković, Fraser, Just — *Practical Mutation Testing at Scale: A View from Google* | **IEEE TSE 2022** (arXiv preprint 2021) | <https://arxiv.org/abs/2102.11378> | "developers at Google initially classified **85% of reported mutants as unproductive**"; diff-based mutation on changed code; mutants capped per line and per review | ✅ verified |
| Petrović & Ivanković — *State of Mutation Testing at Google* | ICSE-SEIP 2018 | <https://research.google/pubs/state-of-mutation-testing-at-google/> | Distinct earlier paper — **do not merge** with the TSE 2022 work above | ⚠️ not re-verified |
| Beller, Wong, Bader, Scott, Machalica, Chandra — *What It Would Take to Use Mutation Testing in Industry — A Study at Facebook* | ICSE-SEIP 2021 | <https://arxiv.org/abs/2010.13464> | Learned error-inducing patterns from real Java errors and operational anomalies; >15,000 mutants, **more than half survived** Facebook's suite; 26-developer case study | ✅ verified |
| Madeyski, Orzeszyna, Torkar, Józala — *Overcoming the Equivalent Mutant Problem* | IEEE TSE 2014 (cited as 2013) | <https://doi.org/10.1109/TSE.2013.44> | Equivalent-mutant rate **4%–39%** — verified as quoted in Tian et al. ISSTA 2024 | ✅ verified via citing paper |
| Budd & Angluin — *Two notions of correctness and their relation to testing* | Acta Informatica, 1982 | <https://doi.org/10.1007/BF00263171> | Mutant equivalence is undecidable in the general case | ⚠️ not re-verified (textbook-level result) |
| DeMillo, Lipton, Sayward — *Hints on Test Data Selection* | IEEE Computer, 1978 | <https://doi.org/10.1109/C-M.1978.218136> | Competent programmer hypothesis; coupling effect | ⚠️ not re-verified (textbook-level result) |
| *Mutation Testing as a QA Technique in a Fintech Company: An Experience Report* (authors not verified) | SBQS 2024 | <https://dl.acm.org/doi/10.1145/3701625.3701629> | **Recommended peer-reviewed replacement** for the unverified Theodo banking case | ⚠️ not yet read in full |

#### Reconciliation note — Banik et al. (80.2% weak oracles) vs Yoshimoto et al. (higher assertion density)

These two findings look contradictory and will be raised in Q&A. They are not, because they measure different things:

- **Different unit.** Yoshimoto et al. count assertions per *test method*. Banik et al. classify the oracle *strength* of a whole *patch*.
- **Different question.** "How many assertions are there" is not "what do those assertions verify". The weak categories W1–W5 include assertions that only confirm the code ran, non-null checks, mock/interaction checks, and snapshot comparisons — all of which raise assertion count without verifying output correctness.
- **They agree on the gap.** Yoshimoto et al. explicitly name mutation testing as the measurement needed to close it. Quote their future-work sentence directly; it is the strongest available answer to this objection.

Sampling caveats on the 80.2%: repositories filtered to 100+ stars, and OpenAI Codex contributes 65% of the PRs, so the aggregate skews toward Codex behaviour. When precision matters, cite the per-agent range (18–67% strong-oracle rate on newly created files) rather than the pooled figure.

### Official tool documentation

| Doc | URL | Verified claim | Status |
| --- | --- | --- | --- |
| Stryker — Mutant states and metrics | <https://stryker-mutator.io/docs/mutation-testing-elements/mutant-states-and-metrics/> | States: Pending / Killed / Survived / No coverage / Timeout / Runtime error / Compile error / Ignored. `Detected = killed + timeout`. **`Mutation score = detected / valid × 100`**; **`Mutation score based on covered code = detected / covered × 100`**. Timeout is explicitly "counted as detected". Compile/runtime errors are **not** represented in the score; Ignored does not count against it | ✅ verified |
| StrykerJS — Configuration | <https://stryker-mutator.io/docs/stryker-js/configuration/> | Mutation range syntax `"src/app.js:1-11"` (and `:startLine[:startColumn]-endLine[:endColumn]`) is **official and documented**. ⚠️ Constraint: a mutation range **cannot be combined with a globbing expression on the same line** | ✅ verified |
| StrykerJS — Supported mutators | <https://stryker-mutator.io/docs/mutation-testing-elements/supported-mutators/> | Operator catalogue | ⚠️ not re-verified |
| PIT — Basic concepts / Mutators / FAQ | <https://pitest.org/quickstart/basic_concepts/> | Survived vs no-coverage distinction; timeout instability | ⚠️ not re-verified |

### Withdrawn / do not cite

| Claim | Reason |
| --- | --- |
| "Theodo banking microservice: 96% line / 93% branch coverage, 34% mutation score" | No primary URL located (30 Jul 2026). Consultancy-blog provenance, not peer-reviewed. Previously mis-attributed to MutGen in `seminar_presentation_design.md` — MutGen contains no such case. Replace with the SBQS 2024 fintech report, or drop in favour of the team's own measured e-Shop numbers |
| Diffblue vendor benchmarks (MS 60–89%, Copilot 5–29% coverage, 65% pass rate) | Vendor's own studies, no independent reproduction. If used at all, label explicitly as vendor-reported |
| "MutGen lifts LLM-generated tests from 4% to 89.5% mutation score" | **Unit mismatch — never pair these two numbers.** 4% is one subject (`id_81`) under vanilla prompting; 89.5% is the MutGen average over 104 subjects. The honest apples-to-apples pair is Gen_vanilla → MutGen: 77.9% → 89.5% (HumanEval-Java) and 69.9% → 89.1% (Leetcode-Java) |
| "MutGen beats EvoSuite by 28.8 / 51.3 percentage points" | Those figures are **relative** improvements. In percentage points it is +20.0 and +30.2 |
| "EvoSuite baselines 65.0% / 73.4% mutation score" (earlier draft of this file) | **Wrong — corrected 01/08/2026.** Table I gives EvoSuite 69.5% / 58.9%. The 73.4% figure appears in no column; it was likely a coverage value misfiled as mutation score |
| "MutGen shows no significant coverage difference between approaches" (earlier draft) | Overstated. The paper reports coverage differences **below 3 percentage points**, and on Leetcode-Java EvoSuite achieves the *highest* coverage of all four approaches. No Wilcoxon test on coverage was located. State the spread, not a significance claim |
