...
app.post("/api/apply-coupon", (req, res) => {
  ...
  db.get("SELECT * FROM coupons WHERE code = ? AND is_active = 1", [code], (err, coupon) => {
    ...
    if (total_amount > coupon.min_order_amount) {
      ...
      if (stryMutAct_9fa48("3") ? expiry >= now : stryMutAct_9fa48("2") ? expiry <= now : stryMutAct_9fa48("1") ? false : stryMutAct_9fa48("0") ? true : (stryCov_9fa48("0", "1", "2", "3"), expiry < now)) {
        return res.status(400).json({
          error: "Mã giảm giá đã hết hạn"
        });
      }
      if (user_id) {
        db.get("SELECT COUNT(*) as usage_count FROM coupon_usage WHERE coupon_id = ? AND user_id = ?", [coupon.id, user_id], (err, result) => {
